...
//struct str
int bignum_cmp_1(struct bn* a,char cmp0, char cmp1, struct bn* b)
{
  char cmp[2] = {cmp0, cmp1};
  require(a, "a is null");
  require(b, "b is null");
  char* gt    = ">";
  char* lt    = "<";
  ...
  while ((i != 0)&&(result==0xBADF00D));
  if(result==0xBADF00D)
    result = EQUAL;

  if((cmp[0]==gtoet[0])&&(cmp[1]==gtoet[1]))
    if((result==LARGER)||(result==EQUAL))
      return 1;
    else
      return 0;
  else if((cmp[0]==ltoet[0])&&(cmp[1]==ltoet[1]))
    if((result==SMALLER)||(result==EQUAL))
      return 1;
    else
      return 0;
  else if((cmp[0]==e[0])&&(cmp[1]==e[1]))
    if(result==EQUAL)
      return 1;
    else
      return 0;
