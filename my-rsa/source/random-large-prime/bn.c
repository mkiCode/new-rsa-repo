...
void bignum_rshift_nocopy(struct bn* b, int nbits)
{
  //require(a, "a is null");
  require(b, "b is null");
  require(nbits >= 0, "no negative shifts");
...
int bignum_is_odd(struct bn* a)
{
  if(((uint32_t)(a->array[0]))%2)
    return 1;
  else
    return 0;
}
