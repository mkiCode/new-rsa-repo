...
void main(void)
{
  ...
  char prime_p[size];
  FILE* file;
  file = fopen("../../results/p.txt","r");
  fread(prime_p, sizeof(char),temp, file);
  fclose(file);
  //printf("%s\n", prime_p);

  char prime_q[size];
  file = fopen("../../results/q.txt","r");
  ...
  file = fopen("../../results/eulers_totient.txt", "w");
